...
export default function SupplierSidebar() {
  ...
  const menuItems = [
    { href: '/supplier/dashboard', label: 'Tableau de bord', icon: <LayoutDashboard className="h-5 w-5" /> },
    { href: '/supplier/offers', label: 'Offres', icon: <Tag className="h-5 w-5" /> },
    { href: '/supplier/listings', label: 'Listings PDF', icon: <FileText className="h-5 w-5" /> },
    { href: '/supplier/subscription', label: 'Abonnement', icon: <CreditCard className="h-5 w-5" /> },
  ];
      ...
      <div className="mb-8 pt-2">
        <div className="flex items-center space-x-3">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-sm text-green-600 font-medium mt-0.5">Espace Fournisseur</span>
          </div>
        </div>
      </div>
          ...
          <>
            <Link href="/supplier/profile" className="flex items-center p-3 text-green-800 hover:bg-green-200 rounded-lg transition-colors mb-2">
              ...
              <div className="ml-3 flex-1 min-w-0">
                <p className="text-sm font-medium truncate">{profile.company_name || 'Fournisseur'}</p>
